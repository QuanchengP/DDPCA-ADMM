...
public:
	...
	std::vector<double> deltZlen;//height of the complete upper/lower block
	std::vector<double> uppeZlen;//z coordinate of the complete upper block
	...
	std::vector<CURVEDS> bmupSurf_t;//rigid translation of bmupSurf
...
long BLOCK::ESTA_SURF(){
	...
	tempTran << 0.0, 0.0, - uppeZlen[0];
	bmupSurf_t[0].RIGI_ROTR(Eigen::Matrix3d::Identity(), tempTran);
	bmupSurf_t[1] = bmupSurf[0];
	tempTran << 0.0, 0.0, - deltZlen[0];
	bmupSurf_t[1].RIGI_ROTR(Eigen::Matrix3d::Identity(), tempTran);
	//
	...
	tempTran << 0.0, 0.0, - uppeZlen[1];
	bmupSurf_t[2].RIGI_ROTR(Eigen::Matrix3d::Identity(), tempTran);
	bmupSurf_t[3] = bmupSurf[2];
	tempTran << 0.0, 0.0, - deltZlen[1];
	bmupSurf_t[3].RIGI_ROTR(Eigen::Matrix3d::Identity(), tempTran);
	//
	...
	tempTran << 0.0, 0.0, - uppeZlen[2];
	bmupSurf_t[4].RIGI_ROTR(Eigen::Matrix3d::Identity(), tempTran);
	bmupSurf_t[5] = bmupSurf[4];
	tempTran << 0.0, 0.0, - deltZlen[2];
	bmupSurf_t[5].RIGI_ROTR(Eigen::Matrix3d::Identity(), tempTran);
	//
	...
	for(long tg = 0; tg < blocSurf.size(); tg ++){
		...
		double totaZlen = uppeZlen[tb] - deltZlen[tb];
		//nodes
		for(long ti = 0; ti <= diviReal[0]; ti ++){
			...
			for(long tj = 0; tj <= diviReal[1]; tj ++){
				...
				for(long tk = 0; tk <= diviReal[2]; tk ++){
					...
					double zCoo = deltZlen[tb] + totaZlen / diviNumb_es[tb] * (double)tk_real;
					if(tb == 1){
						zCoo += leng[0];
					}
					else if(tb == 2){
...
long BLOCK::MESH(){
	ESTA_SURF();
	//
	multGrid.resize(3 * domaNumb[0] * domaNumb[1] * domaNumb[2] + 3 * 2);
	#pragma omp parallel for
		...
		if(tb <= 2){
			...
			for(long ti = 0; ti <= diviReal[0]; ti ++){
				...
				for(long tj = 0; tj <= diviReal[1]; tj ++){
					...
					for(long tk = 0; tk <= diviReal[2]; tk ++){
						long tk_real = tg_2 * diviReal[2] + tk;
						double zCoo = deltZlen[tb] + totaZlen / diviNumb[tb] * (double)tk_real;
						if(tb == 1){
							zCoo += leng[0];
						}
						else if(tb == 2){
			...
			double zlenMini = deltZlen[tb];
			double zlenMaxi = uppeZlen[tb];
			if(tb == 1){
				zlenMini += leng[0];
				zlenMaxi += leng[0];
			}
			...
			long tg_temp = tg - 3 * (domaNumb[0] * domaNumb[1] * domaNumb[2]);
			tb = tg_temp / 2;
			long tg_b = tg_temp % 2;
			//nodes
			long diviReal = 1;
			long tempNode[diviNumb[0] + 1][diviNumb[1] + 1][1 + 1];
			for(long ti = 0; ti <= diviNumb[0]; ti ++){
				double xCoo = - leng[tb] / 2.0 + leng[tb] / diviNumb[tb] * (double)ti;
				for(long tj = 0; tj <= diviNumb[1]; tj ++){
					double yCoo = - leng[tb] / 2.0 + leng[tb] / diviNumb[tb] * (double)tj;
					for(long tk = 0; tk <= diviReal; tk ++){
						double zCoo = deltZlen[tb] / diviReal * (double)tk;
						if(tg_b == 1){
							zCoo += uppeZlen[tb];
						}
						if(tb == 1){
			...
			for(long tr = 0; tr < globLeve; tr ++){
				...
				for(long ti = 0; ti < multGrid[tg].elemVect.size(); ti ++){
					...
					spliElem.insert(ti);
					multGrid[tg].elemVect[ti].refiPatt = 1;
				}
				planSurf.clear();
				multGrid[tg].REFINE(spliElem, spliFlag, planSurf);
			}
...
long BLOCK::SOLVE(long appsCont){
	...
	//
	searCont.resize(xyzN[0] + xyzN[1] + xyzN[2] + xyzN[3] + xyzN[4]);
	penaFact_n.resize(xyzN[0] + xyzN[1] + xyzN[2] + xyzN[3] + xyzN[4]);
	penaFact_f.resize(xyzN[0] + xyzN[1] + xyzN[2] + xyzN[3] + xyzN[4]);
	fricCoef.resize(xyzN[0] + xyzN[1] + xyzN[2] + xyzN[3] + xyzN[4]);
	contBody.resize(xyzN[0] + xyzN[1] + xyzN[2] + xyzN[3] + xyzN[4]);
	for(long tb = 0; tb < 3; tb ++){
	...
	long contBase = xyzN[0] + xyzN[1] + xyzN[2];
	long domaBase = 3 * domaNumb[0] * domaNumb[1] * domaNumb[2];
	for(long tb = 0; tb < 3; tb ++){
		for(long bu = 0; bu < 2; bu ++){
			for(long tg_0 = 0; tg_0 < domaNumb[0]; tg_0 ++){
				for(long tg_1 = 0; tg_1 < domaNumb[1]; tg_1 ++){
					long tg_m = tb * domaNumb[0] * domaNumb[1] * domaNumb[2] 
						+ tg_0 * domaNumb[1] * domaNumb[2] + tg_1 * domaNumb[2] 
						+ (bu == 0 ? 0 : domaNumb[2] - 1);
					contBody[contBase] = {tg_m, domaBase + tb * 2 + bu};
					contBase ++;
				}
			}
		}
	}
	contBody[contBase + 0] = {domaBase + 1, domaBase + 2};
	contBody[contBase + 1] = {domaBase + 3, domaBase + 4};
	...
	for(long ts = 0; ts < searCont.size(); ts ++){
		...
		if(ts < xyzN[0]){
			...
			mastSurf = &(blocSurf[contBody[ts][0]][4 + tempSunu % 2]);
