...
public:
	...
	//update whdeAucu
	long UPDA_WHDE(const std::vector<long> &inpuNode, const COOR &outpCoor, long whee_tv);
	//update whdeAucu_midd
	long UPDA_WHDE_MIDD(const std::vector<long> &inpuNode, const COOR &outpCoor, long whee_tv);
	...
	std::vector<std::vector<CURVEDS>> wodeAucu;//worm decomposition auxiliary CURVEDS
	CURVEDS whdeAucu;//wheel decomposition auxiliary CURVEDS
...
long DEHW::COOR_AVER(const std::vector<COOR> &inpuCoor, COOR &outpCoor){
...
long DEHW::COOR_AVER_1(const std::vector<COOR> &inpuCoor, COOR &outpCoor){
	COOR cyliCoor(0.0, 0.0, 0.0);
	long tempFlag_0 = 0;
	long tempFlag_1 = 0;
	bool inneHubf = true;
...
long DEHW::SUBR_COLO_WORM(long tg){
	if(tg == -1){
...
long DEHW::SUBR_COLO_WHEE(long tg){
	if(tg == -1){
...
long DEHW::WORM_MESH(long isnoDode){
	OUTPUT_TIME("DEHW::WORM_MESH");
	//
...
long DEHW::WHEE_MESH_DD(){
	OUTPUT_TIME("DEHW::WHEE_MESH_DD");
	//
	...
	for(long tw = dehwSurf.gridNumb[0][6]; tw < multGrid.size(); tw ++){
		...
		for(long ti = 0; ti <= numbFace; ti ++){
			...
			for(long tj = 0; tj <= dehwSurf.gridNumb[1][1]; tj ++){
				for(long tk = tk_star; tk <= tk_endi; tk ++){
					...
					blocPoin[0][ti][tj][tk - tk_star] = 
						multGrid[tw].TRY_ADD_NODE({tempCoor(0), tempCoor(1), tempCoor(2)});
					if(toot_tw == 0 && leri_tw == 1 && tk == tk_endi){
						COOR inpuCoor = {tempCoor(0), tempCoor(1), tempCoor(2)};
						whdeAucu.INSERT(tj * whdeFact_0, ti * whdeFact_1, inpuCoor);
					}
					if(toot_tw == 0 && leri_tw == 0 && tk == tk_endi){
						COOR inpuCoor = {tempCoor(0), tempCoor(1), tempCoor(2)};
						whdeAucu_midd.INSERT(tj * whdeFact_0, ti * whdeFact_1, inpuCoor);
					}
				}
			}
			//1
		...
		if(toot_tw  == 0 && leri_tw == 1){
			whdeAucu.RIGI_ROTR(rotaMatr, tranVect);
			whdeAucu.RIGI_ROTR(wheeRota, wheeTran);
		}
		if(toot_tw  == 0 && leri_tw == 0){
			whdeAucu_midd.RIGI_ROTR(rotaMatr, tranVect);
			whdeAucu_midd.RIGI_ROTR(wheeRota, wheeTran);
		}
	}
	return 1;
}

long DEHW::WHEE_MESH_NODD(){
	OUTPUT_TIME("DEHW::WHEE_MESH_NODD");
	//
	long numbFace = dehwSurf.gridNumb[1][4];
	long tw_real = 1;
...
long DEHW::UPDA_WODE(const std::vector<long> &inpuNode, 
	const COOR &outpCoor, long face_tw, long face_sub, long worm_tv){
	long ti_aver = 0;
	long tj_aver = 0;
	bool tempFlag = true;
...
long DEHW::UPDA_WHDE(const std::vector<long> &inpuNode, const COOR &outpCoor, long whee_tv){
	long ti_aver = 0;
	long tj_aver = 0;
	bool tempFlag = true;
	for(long ic = 0; ic < inpuNode.size(); ic ++){
		auto iterNoco = multGrid[whee_tv].nodeCoor.find(inpuNode[ic]);
		auto iterPoin = whdeAucu.poinIndi.find(iterNoco->second);
		if(iterPoin == whdeAucu.poinIndi.end()){
			tempFlag = false;
			break;
		}
		ti_aver += (iterPoin->second)[0];
...
long DEHW::UPDA_WHDE_MIDD(const std::vector<long> &inpuNode, const COOR &outpCoor, long whee_tv){
	long ti_aver = 0;
	long tj_aver = 0;
	bool tempFlag = true;
	for(long ic = 0; ic < inpuNode.size(); ic ++){
		auto iterNoco = multGrid[whee_tv].nodeCoor.find(inpuNode[ic]);
		auto iterPoin = whdeAucu_midd.poinIndi.find(iterNoco->second);
		if(iterPoin == whdeAucu_midd.poinIndi.end()){
			tempFlag = false;
			break;
		}
		ti_aver += (iterPoin->second)[0];
...
long DEHW::CONT_INTE_DD(){
	...
	std::vector<std::vector<bool>> isnoRefi(4, std::vector<bool>(3));
	...
	//****************************************************************************************
	OUTPUT_TIME("DEHW::CONT_INTE_DD contact surface");
	...
	//****************************************************************************************
	OUTPUT_TIME("DEHW::CONT_INTE_DD surfaces between domains of worm tooth");
	Eigen::Matrix3d wormRota_0;
	wormRota_0 << cos(analAngl[0]), sin(analAngl[0]), 0.0, 
		-sin(analAngl[0]), cos(analAngl[0]), 0.0, 
		0.0, 0.0, 1.0;
	...
	//****************************************************************************************
	OUTPUT_TIME("DEHW::CONT_INTE_DD surfaces between worm tooth");
	#pragma omp parallel for
	for(long tv = 0; tv < dehwSurf.gridNumb[0][6] - dehwSurf.circNumb; tv ++){
		if(tv == 0){
	...
	//****************************************************************************************
	OUTPUT_TIME("DEHW::CONT_INTE_DD surfaces between domains of wheel tooth");
	buckNumb = {(dehwSurf.gridNumb[1][1] + dehwSurf.gridNumb[1][3]) 
	...
	for(long tv_0 = 0; tv_0 < dehwSurf.gridNumb[1][5]; tv_0 ++){
		for(long tv_1 = 0; tv_1 < dehwSurf.gridNumb[1][6] - 1; tv_1 ++){
			//
			CURVEDS tempCusu = whdeAucu_midd;
			Eigen::Matrix3d rotaMatr;
	...
	//****************************************************************************************
	OUTPUT_TIME("DEHW::CONT_INTE_DD surfaces between wheel teeth");
...
long DEHW::CONT_INTE_NODD(){
	...
	cusuTabl_0.clear();
	//****************************************************************************************
	OUTPUT_TIME("DEHW::CONT_INTE_NODD contact surface");
	long totaLeve = dehwSurf.globInho + dehwSurf.globHomo + dehwSurf.locaLeve;
	std::vector<long> buckNumb = {
